...
export default async function handler(req, res) {
    ...
    try {
        ...
        // 🚀 DUAL ENGINE: DATA PAYLOAD (ANDROID SAFE) + APNS ALERT (APPLE iOS SAFE)
        const message = {
            topic: targetTopic,
            // 🤖 Android data payload handled by MyFCMService with zero crash
            data: {
                title: String(title),
                body: String(body),
            ...
            // 🍏 Apple iOS native lock screen payload
            apns: {
                headers: {
                    "apns-priority": "10",
