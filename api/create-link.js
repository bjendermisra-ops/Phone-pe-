...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Success redirect points back to your receipt.html success callback
        ...
        // STEP 1: Generate OAuth Token securely from PhonePe Identity Manager
        ...
        if (!accessToken) {
            return res.status(500).json({ 
                error: "Failed to generate PhonePe PG V2 OAuth Token. Raw Response: " + JSON.stringify(tokenData) 
            });
        }

        // STEP 2: Create Payment Session using PhonePe V2 Checkout API
        ...
        if (payResponse.status === 200 && payData.redirectUrl) {
            ...
            // DIAGNOSTICS: Yahan hum direct PhonePe ka raw error return kar rahe hain taaki hume reason pata chale!
            return res.status(500).json({ 
                error: "PhonePe PG V2 Pay-link initiation failed. Raw Error: " + JSON.stringify(payData) 
            });
        }

    } catch (error) {
        console.error(error);
        return res.status(500).json({ error: "Server V2 Error: " + error.message });
    }
}
